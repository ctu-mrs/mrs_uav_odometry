...
namespace mrs_odometry
{
  ...
  void Odometry::onInit() {
    ...
    brick_yaw_filter = std::make_shared<MedianFilter>(_brick_yaw_filter_buffer_size, 1000000,
                                                             -1000000, _brick_yaw_filter_max_diff);
  ...
  void Odometry::callbackHectorPose(const geometry_msgs::PoseStampedConstPtr &msg) {
...

    {
      ...
      if (std::pow(hector_pose.pose.position.x - hector_pose_previous.pose.position.x, 2) > 50 ||  std::pow(hector_pose.pose.position.y - hector_pose_previous.pose.position.y, 2) > 50) {

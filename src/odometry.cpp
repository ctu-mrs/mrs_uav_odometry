...
private:
  ...
  double _max_optflow_altitude_     = 0;
  double _max_default_altitude_     = 0;
  double _max_garmin_flight_height_ = 0;
  double _safety_area_height_       = 0;
...
private:
  ...
  std::string gps_origin_frame_id_;
  std::string stable_origin_frame_id_;
  std::string fixed_map_origin_frame_id_;
  ...
  void callbackUWBPose(const geometry_msgs::PoseStampedConstPtr &msg);
  ...
  void               setMaxAltitude(const double desired_max_altitude);
  ...
  nav_msgs::Odometry odom_fixed_map_;
  tf2::Vector3       fixed_map_origin_pos_offset_;
...
void Odometry::onInit() {
  ...
  param_loader.loadParam("altitude/max_allowed_flight_height_garmin", _max_garmin_flight_height_);

  param_loader.loadParam("safety_area/use_safety_area", _use_safety_area_);
  if (_use_safety_area_) {
    param_loader.loadParam("safety_area/max_height", _safety_area_height_);
    max_altitude_ = _safety_area_height_;
  } else {
    ROS_WARN("[Odometry]: NOT USING SAFETY AREA!!!");
  ...
  // subscriber for uwb pose
  if (uwb_active_) {
    sub_uwb_pose_ = nh_.subscribe("uwb_pose_in", 1, &Odometry::callbackUWBPose, this, ros::TransportHints().tcpNoDelay());
  }
...
bool Odometry::isReadyToTakeoff() {
  ...
  if (_estimator_type_takeoff_.type == mrs_msgs::EstimatorType::UWB) {
    if (got_uwb_) {
      return true;
    } else {
      ROS_WARN_THROTTLE(1.0, "[Odometry]: Waiting for UWB pose msg to initialize takeoff estimator");
      return false;
    }
  }
...
void Odometry::mainTimer(const ros::TimerEvent &event) {
  ...
  if (!is_updating_state_) {
    ...
    // correction step for uwb
    if (got_uwb_ && uwb_corr_ready_) {
      auto pos_uwb_x_tmp = mrs_lib::get_mutexed(mutex_uwb_, pos_uwb_x_);
      auto pos_uwb_y_tmp = mrs_lib::get_mutexed(mutex_uwb_, pos_uwb_y_);
      stateEstimatorsCorrection(pos_uwb_x_tmp, pos_uwb_y_tmp, "pos_uwb");
    }
  ...
  if (estimator_type_.type == mrs_msgs::EstimatorType::RTK) {
    ...
    if (!uwb_reliable_ && gps_active_ && gps_reliable_ && got_odom_pixhawk_) {
      ROS_WARN("[Odometry]: UWB not reliable. Switching to GPS type.");
      mrs_msgs::EstimatorType gps_type;
      gps_type.type = mrs_msgs::EstimatorType::GPS;
      if (!changeCurrentEstimator(gps_type)) {
        ROS_ERROR_THROTTLE(1.0, "[Odometry]: Fallback odometry not available. Triggering failsafe.");
    ...
    } else if (!got_odom_pixhawk_ || (!got_range_ && garmin_enabled_) || (!got_uwb_)) {
      ROS_INFO_THROTTLE(1, "[Odometry]: Waiting for data from sensors - received? pixhawk: %s, ranger: %s, global position: %s, uwb: %s",
                        got_odom_pixhawk_ ? "TRUE" : "FALSE", got_range_ ? "TRUE" : "FALSE", got_pixhawk_utm_ ? "TRUE" : "FALSE", got_uwb_ ? "TRUE" : "FALSE");
      if (got_lateral_sensors_ && !failsafe_called_) {
        ROS_ERROR_THROTTLE(1.0, "[Odometry]: No fallback odometry available. Triggering failsafe.");
        std_srvs::Trigger failsafe_out;
        ser_client_failsafe_.call(failsafe_out);
  ...
  if (!odometry_published_) {
    ...
    for (auto &estimator : _lateral_estimators_) {
      ...
      if (estimator.second->getName() == "OPTFLOW" || estimator.second->getName() == "HECTOR" || estimator.second->getName() == "ALOAM" ||
          ...
          estimator.second->getName() == "ICP" || estimator.second->getName() == "ALOAMREP" || estimator.second->getName() == "UWB") {
    ...
    fixed_map_origin_pos_offset_.setX(0.0);
    fixed_map_origin_pos_offset_.setY(0.0);
    fixed_map_origin_pos_offset_.setZ(0.0);
    fixed_map_origin_rot_offset_ = mrs_lib::AttitudeConverter(0, 0, 0);

    odom_fixed_map_.header         = odom_main.header;
    odom_fixed_map_.child_frame_id = odom_main.child_frame_id;
  ...
  if (_publish_fused_odom_) {
    ...
    if (odom_main.header.frame_id != last_local_name_) {
      ...
      tf2::fromMsg(odom_main.pose.pose.position, v1);
      tf2::fromMsg(odom_fixed_map_.pose.pose.position, v2);
      pos_diff                     = v1 - v2;
      fixed_map_origin_pos_offset_ = pos_diff;

      q1                           = mrs_lib::AttitudeConverter(odom_main.pose.pose.orientation);
      q2                           = mrs_lib::AttitudeConverter(odom_fixed_map_.pose.pose.orientation);
      rot_diff                     = q2 * q1.inverse();
      fixed_map_origin_rot_offset_ = rot_diff;
      fixed_map_origin_rot_offset_.normalize();
    }
    ...
    odom_fixed_map_ = applyOdomOffset(odom_main, fixed_map_origin_pos_offset_, fixed_map_origin_rot_offset_);
    /* ROS_WARN("[Odometry]: after stable_q: %f, %f, %f, %f", odom_local_.pose.pose.orientation.x, odom_local_.pose.pose.orientation.y, */
    /* odom_local_.pose.pose.orientation.z, odom_local_.pose.pose.orientation.w); */
    odom_fixed_map_.header.frame_id = fixed_map_origin_frame_id_;
...
void Odometry::topicWatcherTimer(const ros::TimerEvent &event) {
  ...
  // uwb pose (corrections of lateral kf)
  interval = ros::Time::now() - uwb_pose_last_update_;
  if (got_uwb_ && interval.toSec() > 0.1) {
    ROS_WARN("[Odometry]: UWB pose not received for %f seconds.", interval.toSec());
    if (got_uwb_ && interval.toSec() > 1.0) {
      uwb_reliable_ = false;
    }
  }
}
...
void Odometry::callbackMavrosOdometry(const nav_msgs::OdometryConstPtr &msg) {
  ...
  if (gps_active_) {
    ...
    geometry_msgs::Vector3Stamped global_vel;
    auto                          response = transformer_->transformSingle(local_vel, gps_origin_frame_id_);
    if (response) {
...
void Odometry::callbackUWBPose(const geometry_msgs::PoseStampedConstPtr &msg) {

  if (!is_initialized_)
    return;

  mrs_lib::Routine    profiler_routine = profiler_.createRoutine("callbackUWBPose");
  mrs_lib::ScopeTimer timer            = mrs_lib::ScopeTimer("Odometry::callbackUWBPose", scope_timer_logger_, scope_timer_enabled_);

  uwb_pose_last_update_ = ros::Time::now();

  {
    ...
    if (got_uwb_) {
      ...
      uwb_pose_previous_ = uwb_pose_;
      uwb_pose_          = *msg;

    } else {

      uwb_pose_previous_ = *msg;
      uwb_pose_          = *msg;

      // Wait for uwb convergence to initial position
      if (c_uwb_init_msgs_++ < 10) {
        ROS_INFO("[Odometry]: UWB pose #%d - x: %f y: %f", c_uwb_init_msgs_, uwb_pose_.pose.position.x, uwb_pose_.pose.position.y);
        return;
      }
      ...
      Vec2 zero_state = zero_state.Zero();
      Vec2 pos;
      pos << uwb_pose_.pose.position.x, uwb_pose_.pose.position.y;
      for (auto &estimator : _lateral_estimators_) {
        if (estimator.first == "UWB") {
          estimator.second->setState(0, pos);
          estimator.second->setState(1, zero_state);
          estimator.second->setState(2, zero_state);
        }
    ...
    if (uwb_reliable_) {
      ...
      // Detect jump since previous pose
      if (std::pow(uwb_pose_.pose.position.x - uwb_pose_previous_.pose.position.x, 2) > 4 ||
          std::pow(uwb_pose_.pose.position.y - uwb_pose_previous_.pose.position.y, 2) > 4) {
        ROS_WARN("[Odometry]: Jump detected in UWB pose. Not reliable");
      ...
      if (current_lat_estimator_->getName() == "UWB") {
        ...
        current_lat_estimator_->getState(1, vel_vec);
        if (vel_vec(0) > 5 || vel_vec(1) > 5) {
          ROS_WARN("[Odometry]: UWB velocity too large - x: %f, y: %f. Not reliable.", vel_vec(0), vel_vec(1));
  ...
  if (!isTimestampOK(uwb_pose_.header.stamp.toSec(), uwb_pose_previous_.header.stamp.toSec())) {
    ROS_DEBUG_THROTTLE(1.0, "[Odometry]: UWB pose timestamp not OK, not fusing correction.");
    return;
  }
  ...
  if (!got_lateral_sensors_) {
    ROS_WARN_THROTTLE(1.0, "[Odometry]: Not fusing UWB pose. Waiting for other sensors.");
    return;
  }
  ...
  if (res) {
    ...
    ROS_ERROR_THROTTLE(1.0, "[Odometry]: Could not transform UWB pose to %s", uwb_frame_id_.c_str());
    return;
  }
  ...
  // Set innovation variable if current estimator is UWB
  if (current_lat_estimator_->getName() == "UWB") {
    Vec2 pos_vec, innovation;
    current_lat_estimator_->getState(0, pos_vec);
    ...
    {
      std::scoped_lock lock(mutex_uwb_);
      innovation(0) = pos_uwb_x_ - pos_vec(0);
      innovation(1) = pos_uwb_y_ - pos_vec(1);
    }
  ...
  ROS_INFO_ONCE("[Odometry]: Fusing UWB position");
}
//}
...
void Odometry::callbackGarmin(const sensor_msgs::RangeConstPtr &msg) {
  ...
  auto res = transformer_->transformSingle(garmin_point, fcu_untilted_frame_id_);
...
void Odometry::callbackSonar(const sensor_msgs::RangeConstPtr &msg) {
  ...
  auto res = transformer_->transformSingle(sonar_point, fcu_untilted_frame_id_);
...
void Odometry::callbackPixhawkUtm(const sensor_msgs::NavSatFixConstPtr &msg) {
  ...
  gps_local_odom.header.frame_id = gps_origin_frame_id_;
    ...
    if (_gps_fallback_estimator_ == "OPTFLOW") {
      ...
      if (optflow_active_ && got_optflow_) {
        ...
        if (have_alt) {
          ...
          reference_srv.request.header.frame_id      = fcu_untilted_frame_id_;
...
bool Odometry::callbackChangeOdometrySource(mrs_msgs::String::Request &req, mrs_msgs::String::Response &res) {
  ...
  if (type == "OPTFLOW") {
    ...
    if (garmin_enabled_) {
      ...
      desired_alt_estimator.type = mrs_msgs::AltitudeType::BARO;
    }
...
void Odometry::callbackReconfigure([[maybe_unused]] mrs_uav_odometry::odometry_dynparamConfig &config, [[maybe_unused]] uint32_t level) {
  ...
  for (auto &estimator : _lateral_estimators_) {
    ...
    estimator.second->setR(config.R_pos_uwb, map_measurement_name_id_.find("pos_uwb")->second);
...
bool Odometry::changeCurrentEstimator(const mrs_msgs::EstimatorType &desired_estimator) {
  ...
  if (target_estimator.type == mrs_msgs::EstimatorType::OPTFLOW) {
    ...
    setMaxAltitude(_max_optflow_altitude_);
    ...
    /* UWB //{ */
  } else if (target_estimator.type == mrs_msgs::EstimatorType::UWB) {

    if (!uwb_active_) {
      ROS_ERROR("[Odometry]: Cannot transition to UWB type. UWB estimator not active.");
      return false;
    }

    if (!got_uwb_ && is_ready_to_takeoff_) {
      ROS_ERROR("[Odometry]: Cannot transition to UWB type. No new UWB msgs received.");
      return false;
    }
...
bool Odometry::changeCurrentAltitudeEstimator(const mrs_msgs::AltitudeType &desired_estimator) {
  ...
  if (target_estimator.type == mrs_msgs::AltitudeType::HEIGHT) {
    ...
    if (!garmin_enabled_) {
      ...
      ROS_WARN_THROTTLE(1.0,
                        "[Odometry]: Tried to switch altitude estimator to HEIGHT without garmin_enabled_. Do you have garmin_down in SENSORS in .bashrc?");
      return false;
    }

    setMaxAltitude(_max_garmin_flight_height_);
  }
  /*//}*/
  ...
  if (target_estimator.type == mrs_msgs::AltitudeType::BRICK) {
    ...
    setMaxAltitude(_brick_max_valid_alt_);
  }
  ...
  /* set max altitude of estimators that allow it //{*/
  if (target_estimator.type != mrs_msgs::AltitudeType::HEIGHT && target_estimator.type != mrs_msgs::AltitudeType::PLANE &&
      target_estimator.type != mrs_msgs::AltitudeType::BRICK) {
...
/* setMaxAltitude() //{*/
void Odometry::setMaxAltitude(const double desired_max_altitude) {

  double new_altitude = desired_max_altitude;
  ;

  // saturate to safety area height if safety area is used
  if (_use_safety_area_) {

    if (new_altitude > _safety_area_height_) {

      ROS_WARN_THROTTLE(1.0, "[Odometry]: Desired max altitude %.2f is higher than safety area height %.2f. Limiting max altitude to safety area height.",
                        new_altitude, _safety_area_height_);
      new_altitude = _safety_area_height_;
    }

    // if safety area is not used saturate to max default altitude
  } else {

    if (new_altitude > _max_default_altitude_) {

      ROS_WARN_THROTTLE(1.0, "[Odometry]: Desired max altitude %.2f is higher than max default altitude %.2f. Limiting max altitude to max default altitude.",
                        new_altitude, _max_default_altitude_);
      new_altitude = _max_default_altitude_;
    }
  }

  // saturate to max garmin flight height is altitude estimator is height
  if (current_alt_estimator_name_ == "HEIGHT" && new_altitude > _max_garmin_flight_height_) {

    ROS_WARN_THROTTLE(1.0, "[Odometry]: Current altitude estimator is HEIGHT. Limiting max altitude to %.2f.", _max_garmin_flight_height_);
    new_altitude = _max_garmin_flight_height_;
  }

  mrs_lib::set_mutexed(mutex_max_altitude_, new_altitude, max_altitude_);
  ROS_WARN("[Odometry]: Setting max_altitude to %.2f", new_altitude);
}
/*//}*/

...
void Odometry::mainTimer(const ros::TimerEvent &event) {
  ...
  if (_publish_fused_odom) {
    ...
    if (!isEqual(current_hdg_estimator_name, "PIXHAWK")) {
    ...
    // Get inverse trasnform
    tf2::Transform tf_stable_inv        = mrs_odometry::tf2FromPose(odom_stable.pose.pose);
...
void Odometry::callbackReconfigure([[maybe_unused]] mrs_odometry::odometry_dynparamConfig &config, [[maybe_unused]] uint32_t level) {
  ...
  ROS_INFO(
      ...
      "R_rate_optflow: %f\n"
      "R_rate_icp: %f\n",
      config.R_yaw_compass, config.R_yaw_hector, config.R_yaw_aloam, config.R_yaw_brick, config.R_yaw_vio, config.R_yaw_vslam, config.R_yaw_lidar,

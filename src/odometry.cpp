...
private:
  ...
  ros::Publisher pub_pavel_reset_;
...
private:
  ...
  ros::Subscriber sub_pavel_pose_;
...
private:
  ...
  ros::ServiceServer ser_reset_pavel_;
  ros::ServiceServer ser_reliable_pavel_;
...
private:
  ...
  // Pavel heading msgs
  double                        pavel_yaw_previous;
  std::mutex                    mutex_pavel_hdg;
  ros::Time                     pavel_yaw_last_update;
  std::shared_ptr<MedianFilter> pavel_yaw_filter;
  bool                          _pavel_yaw_median_filter;
  int                           _pavel_yaw_filter_buffer_size;
  double                        _pavel_yaw_filter_max_valid;
  double                        _pavel_yaw_filter_max_diff;
  ...
  // Pavel messages
  std::mutex                    mutex_pavel;
  std::mutex                    mutex_pos_pavel_;
  double                        pos_pavel_corr_x_, pos_pavel_corr_y_;
  geometry_msgs::PoseStamped    pavel_pose;
  geometry_msgs::PoseStamped    pavel_pose_previous;
  ros::Time                     pavel_pose_last_update;
  std::shared_ptr<MedianFilter> pavel_pos_filter_x;
  std::shared_ptr<MedianFilter> pavel_pos_filter_y;
  bool                          _pavel_pos_median_filter;
  int                           _pavel_pos_filter_buffer_size;
  double                        _pavel_pos_filter_max_valid;
  double                        _pavel_pos_filter_max_diff;
  bool                          pavel_reset_called_         = false;
  bool                          _reset_pavel_after_takeoff_ = false;
  Vec2                          pavel_offset_;
  double                        pavel_offset_hdg_;
  ...
  void callbackPavelPose(const geometry_msgs::PoseStampedConstPtr &msg);
  ...
  bool callbackResetPavel([[maybe_unused]] std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res);
  bool callbackReliableHector([[maybe_unused]] std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res);
  bool callbackReliablePavel([[maybe_unused]] std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res);
...
void Odometry::onInit() {
  ...
  pavel_reset_called_         = false;
  _reset_pavel_after_takeoff_ = false;
  pavel_offset_ << 0, 0;
  pavel_offset_hdg_ = 0;
  ...
  _estimator_type_names.push_back(NAME_OF(mrs_msgs::EstimatorType::PAVEL));
  ...
  _heading_type_names.push_back(NAME_OF(mrs_msgs::HeadingType::PAVEL));
  ...
  pavel_reliable             = _lidar_available;
  ...
  // Pavel median filter
  param_loader.load_param("lateral/pavel_pos_median_filter", _pavel_pos_median_filter);
  param_loader.load_param("lateral/pavel_pos_filter_buffer_size", _pavel_pos_filter_buffer_size);
  param_loader.load_param("lateral/pavel_pos_filter_max_valid", _pavel_pos_filter_max_valid);
  param_loader.load_param("lateral/pavel_pos_filter_max_diff", _pavel_pos_filter_max_diff);

  pavel_pos_filter_x =
      std::make_shared<MedianFilter>(_pavel_pos_filter_buffer_size, _pavel_pos_filter_max_valid, -_pavel_pos_filter_max_valid, _pavel_pos_filter_max_diff);
  pavel_pos_filter_y =
      std::make_shared<MedianFilter>(_pavel_pos_filter_buffer_size, _pavel_pos_filter_max_valid, -_pavel_pos_filter_max_valid, _pavel_pos_filter_max_diff);
  ...
  param_loader.load_param("heading/pavel_yaw_filter_buffer_size", _pavel_yaw_filter_buffer_size);
  /* param_loader.load_param("heading/pavel_yaw_filter_max_valid", _pavel_yaw_filter_max_valid); */
  param_loader.load_param("heading/pavel_yaw_filter_max_diff", _pavel_yaw_filter_max_diff);
  ...
  pavel_yaw_filter        = std::make_shared<MedianFilter>(_pavel_yaw_filter_buffer_size, 1000000, -1000000, _pavel_yaw_filter_max_diff);
  ...
  pub_pavel_yaw_   = nh_.advertise<mrs_msgs::Float64Stamped>("pavel_yaw_out", 1);
  ...
  if (_lidar_available) {
    ...
    sub_pavel_pose_  = nh_.subscribe("pavel_pose_in", 1, &Odometry::callbackPavelPose, this, ros::TransportHints().tcpNoDelay());
  }
  ...
  // subscribe for reset pavel service
  ser_reset_pavel_ = nh_.advertiseService("reset_pavel_in", &Odometry::callbackResetPavel, this);

  // subscribe for reliable pavel service
  ser_reliable_pavel_ = nh_.advertiseService("reliable_pavel_in", &Odometry::callbackReliablePavel, this);
  ...
  if (_estimator_type_takeoff.type == mrs_msgs::EstimatorType::PAVEL && !_lidar_available) {
    ROS_ERROR("[Odometry]: The takeoff odometry type %s could not be set. Lidar localization not available. Shutting down.",
              _estimator_type_takeoff.name.c_str());
    ros::shutdown();
  }
  ...
  /* pass current covariances to dynamic reconfigure //{ */
  {
    ...
    current_estimator->getR(last_drs_config.Q_pos_pavel, map_measurement_name_id.find("pos_pavel")->second);
...

  {
    ...
    current_hdg_estimator->getQ(last_drs_config.Q_yaw_pavel, map_hdg_measurement_name_id.find("yaw_pavel")->second);
...
bool Odometry::isReadyToTakeoff() {
  ...
  if (_estimator_type_takeoff.type == mrs_msgs::EstimatorType::PAVEL) {
    if (got_pavel_pose) {
      return true;
    } else {
      ROS_WARN_THROTTLE(1.0, "[Odometry]: Waiting for pavel pose msg to initialize takeoff estimator");
      return false;
    }
  }
  if (_estimator_type_takeoff.type == mrs_msgs::EstimatorType::BRICK) {
...
void Odometry::mainTimer(const ros::TimerEvent &event) {
  ...
  if (_estimator_type.type == mrs_msgs::EstimatorType::RTK) {
    ...
    if (!got_hector_pose || !hector_reliable) {
      if (_optflow_available && got_optflow && current_altitude(mrs_msgs::AltitudeStateNames::HEIGHT) < _max_optflow_altitude) {
        ...
        // Fallback from Pavel Slam
      } else if (_estimator_type.type == mrs_msgs::EstimatorType::PAVEL) {
        if (_hdg_estimator_type.type == mrs_msgs::HeadingType::PAVEL && !pavel_reliable && _gyro_fallback) {
          ROS_WARN_THROTTLE(1.0, "[Odometry]: pavel heading not reliable. Switching to GYRO heading estimator.");
          mrs_msgs::HeadingType desired_estimator;
          desired_estimator.type = mrs_msgs::HeadingType::GYRO;
          desired_estimator.name = _heading_estimators_names[desired_estimator.type];
          changeCurrentHeadingEstimator(desired_estimator);
        }
        if (!got_pavel_pose || !pavel_reliable) {
          if (got_hector_pose && hector_reliable && current_altitude(mrs_msgs::AltitudeStateNames::HEIGHT) < _max_optflow_altitude) {
            ROS_WARN("[Odometry]: PAVEL not reliable. Switching to HECTOR type.");
    ...
    if (!odometry_published) {
      ...
      if ((_estimator_type.type == mrs_msgs::EstimatorType::OPTFLOW || _estimator_type.type == mrs_msgs::EstimatorType::HECTOR ||
           ...
           _estimator_type.type == mrs_msgs::EstimatorType::VSLAM || _estimator_type.type == mrs_msgs::EstimatorType::BRICKFLOW || _estimator_type.type == mrs_msgs::EstimatorType::PAVEL) &&
          use_local_origin_) {
  ...
  void Odometry::callbackTargetAttitude(const mavros_msgs::AttitudeTargetConstPtr &msg) {
    ...
    if (!is_updating_state_) {
      ...
      // correction step for pavel
      stateEstimatorsCorrection(pos_pavel_corr_x_, pos_pavel_corr_y_, "pos_pavel");
  ...
  void Odometry::callbackMavrosOdometry(const nav_msgs::OdometryConstPtr &msg) {
    ...
    // Negate weird simulation position jump glitches (caused by gazebo timestamp glitches - should be handled by isTimestampOK(), this check is for
    // redundancy) (smaller jumps in GPS position are handled by safety control mechanisms)
    if (simulation_) {
  ...
  void Odometry::callbackPavelPose(const geometry_msgs::PoseStampedConstPtr &msg) {

    if (!is_initialized)
      return;

    mrs_lib::Routine profiler_routine = profiler->createRoutine("callbackPavelPose");

    pavel_pose_last_update = ros::Time::now();

    {
      std::scoped_lock lock(mutex_pavel);

      if (got_pavel_pose) {

        pavel_pose_previous = pavel_pose;
        pavel_pose          = *msg;

      } else {

        pavel_pose_previous = *msg;
        pavel_pose          = *msg;
        pavel_yaw_previous  = mrs_odometry::getYaw(pavel_pose.pose.orientation);
      ...
      // Detect jump since previous pose
      if (std::pow(pavel_pose.pose.position.x - pavel_pose_previous.pose.position.x, 2) > 9 ||
          std::pow(pavel_pose.pose.position.y - pavel_pose_previous.pose.position.y, 2) > 9) {
        ROS_WARN_THROTTLE(1.0, "[Odometry]: Jump detected in pavel Slam pose. Not reliable");
        pavel_reliable = false;
      }

      if (isEqual(current_estimator->getName().c_str(), "PAVEL")) {
        Vec2 vel_vec;
        current_estimator->getState(1, vel_vec);
        if (vel_vec(0) > 5 || vel_vec(1) > 5) {
          ROS_WARN_THROTTLE(1.0, "[Odometry]: Pavel Slam velocity too large. Not reliable.");
          pavel_reliable = false;
        }
      }
    }
    ...
    if (!isTimestampOK(pavel_pose.header.stamp.toSec(), pavel_pose_previous.header.stamp.toSec())) {
      ROS_WARN_THROTTLE(1.0, "[Odometry]: pavel pose timestamp not OK, not fusing correction.");
      return;
    }
    ...
    {
      std::scoped_lock lock(mutex_pavel);
      yaw_pavel = mrs_odometry::getYaw(pavel_pose.pose.orientation);
    }

    yaw_pavel = mrs_odometry::unwrapAngle(yaw_pavel, pavel_yaw_previous);
    yaw_pavel += pavel_offset_hdg_;
    pavel_yaw_previous = yaw_pavel;

    // Apply correction step to all heading estimators
    headingEstimatorsCorrection(yaw_pavel, "yaw_pavel");

    yaw_pavel = mrs_odometry::wrapAngle(yaw_pavel);

    mrs_msgs::Float64Stamped pavel_yaw_out;
    pavel_yaw_out.header.stamp    = ros::Time::now();
    pavel_yaw_out.header.frame_id = "local_origin";
    pavel_yaw_out.value           = yaw_pavel;
    pub_pavel_yaw_.publish(pavel_yaw_out);

    ROS_WARN_ONCE("[Odometry]: Fusing yaw from pavel SLAM");

    //////////////////// Fuse Lateral Kalman ////////////////////

    if (!got_lateral_sensors) {
      ROS_WARN_THROTTLE(1.0, "[Odometry]: Not fusing pavel pose. Waiting for other sensors.");
      return;
    }

    double pos_pavel_x, pos_pavel_y;

    {
      std::scoped_lock lock(mutex_pavel);

      pos_pavel_x = pavel_pose.pose.position.x + pavel_offset_(0);
      pos_pavel_y = pavel_pose.pose.position.y + pavel_offset_(1);
    }
    ...
    {
      std::scoped_lock lock(mutex_pavel, mutex_pos_pavel_);

      if (mrs_odometry::isEqual(current_hdg_estimator->getName().c_str(), current_estimator->getName().c_str())) {
        // Corrections and heading are in the same frame of reference
        pos_pavel_corr_x_ = pos_pavel_x;
        pos_pavel_corr_y_ = pos_pavel_y;
      } else {
        // Correct the position by the current heading
        pos_pavel_corr_x_ = pos_pavel_x * cos(yaw - yaw_pavel) - pos_pavel_y * sin(yaw - yaw_pavel);
        pos_pavel_corr_y_ = pos_pavel_x * sin(yaw - yaw_pavel) + pos_pavel_y * cos(yaw - yaw_pavel);
      }
    }

    // Set innoation variable if ccurnet estimator is PAVEL
    if (mrs_odometry::isEqual(current_estimator->getName().c_str(), "PAVEL")) {
      Vec2 pos_vec, innovation;
      current_estimator->getState(0, pos_vec);

      innovation(0) = pos_pavel_corr_x_ - pos_vec(0);
      innovation(1) = pos_pavel_corr_y_ - pos_vec(1);
    ...
    ROS_WARN_ONCE("[Odometry]: Fusing Pavel position");
  }
  //}
  ...
  bool Odometry::callbackChangeOdometrySource(mrs_msgs::String::Request & req, mrs_msgs::String::Response & res) {
    ...
    } else if (std::strcmp(type.c_str(), "PAVEL") == 0) {
      desired_estimator.type     = mrs_msgs::EstimatorType::PAVEL;
      desired_hdg_estimator.type = mrs_msgs::HeadingType::PAVEL;
  ...
  bool Odometry::callbackChangeHdgEstimatorString(mrs_msgs::String::Request & req, mrs_msgs::String::Response & res) {
    ...
    if (std::strcmp(type.c_str(), "PIXHAWK") == 0) {
      ...
      desired_estimator.type = mrs_msgs::HeadingType::PAVEL;
    } else if (std::strcmp(type.c_str(), "LIDAR") == 0) {
  ...
  bool Odometry::callbackResetPavel([[maybe_unused]] std_srvs::Trigger::Request & req, std_srvs::Trigger::Response & res) {
    ...
    LatState2D old_state;
    if (!current_estimator->getStates(old_state)) {
      ROS_WARN_THROTTLE(1.0, "[Odometry]: Could not read current state. Pavel estimator cannot be reset.");
      res.success = false;
      res.message = "Reset of lateral kalman failed";
      return true;
    }
    ...
    // position of odom_stable should contain less drift
    {
      ...
      old_state(0, 1) = odom_stable.pose.pose.position.y;
      for (auto &estimator : m_state_estimators) {
        if (mrs_odometry::isEqual(estimator.first, "PAVEL")) {
          estimator.second->setStates(old_state);
        }
      }
    }

    ROS_WARN("[Odometry]: PAVEL estimator states reset.");

    res.success = true;
    res.message = "Reset of PAVEL estimator successful";
  ...
  /* //{ callbackReliablePavel() */

  bool Odometry::callbackReliablePavel([[maybe_unused]] std_srvs::Trigger::Request & req, std_srvs::Trigger::Response & res) {
    ...
    ROS_WARN("[Odometry]: pavel manually set to reliable.");

    res.success = true;
    res.message = "pavel manually set to reliable";
  ...
  void Odometry::callbackReconfigure([[maybe_unused]] mrs_odometry::lkfConfig & config, [[maybe_unused]] uint32_t level) {
    ...
    for (auto &estimator : m_state_estimators) {
      ...
      estimator.second->setR(config.Q_pos_pavel, map_measurement_name_id.find("pos_pavel")->second);
    ...
    for (auto &estimator : m_heading_estimators) {
      ...
      estimator.second->setQ(config.Q_yaw_pavel, map_hdg_measurement_name_id.find("yaw_pavel")->second);
    }
  }
  //}
  ...
  void Odometry::stateEstimatorsPrediction(double x, double y, double dt) {
    ...
    for (auto &estimator : m_state_estimators) {
      ...
      /* Vec2 pos_vec; */
      /* estimator.second->getState(0, pos_vec); */
      /* ROS_INFO_THROTTLE(1.0, "[Odometry]: %s after prediction with input: %f, dt: %f x: %f", estimator.second->getName().c_str(), input(0), dt,
       * pos_vec(0)); */
    }
  }
  ...
  bool Odometry::changeCurrentEstimator(const mrs_msgs::EstimatorType &desired_estimator) {
    ...
    if (target_estimator.type == mrs_msgs::EstimatorType::OPTFLOW) {
      ...
      max_altitude = _max_default_altitude;
      ROS_WARN("[Odometry]: Setting max_altitude to %f", max_altitude);
 // Pavel SLAM localization type
    } else if (target_estimator.type == mrs_msgs::EstimatorType::PAVEL) {

      if (!_lidar_available) {
        ROS_ERROR("[Odometry]: Cannot transition to PAVEL type. Lidar localization not available in this world.");
        return false;
      }

      if (!got_pavel_pose && is_ready_to_takeoff) {
        ROS_ERROR("[Odometry]: Cannot transition to PAVEL type. No new PAVEL msgs received.");
        return false;
      }
  ...
  bool Odometry::changeCurrentHeadingEstimator(const mrs_msgs::HeadingType &desired_estimator) {
    ...
    if (target_estimator.type != mrs_msgs::HeadingType::PIXHAWK && target_estimator.type != mrs_msgs::HeadingType::GYRO &&
        ...
        target_estimator.type != mrs_msgs::HeadingType::PAVEL && target_estimator.type != mrs_msgs::HeadingType::BRICK && target_estimator.type != mrs_msgs::HeadingType::VIO &&
  ...
  bool Odometry::isValidType(const mrs_msgs::EstimatorType &type) {
    ...
    if (type.type == mrs_msgs::EstimatorType::OPTFLOW || type.type == mrs_msgs::EstimatorType::GPS || type.type == mrs_msgs::EstimatorType::OPTFLOWGPS ||
        ...
        type.type == mrs_msgs::EstimatorType::HECTOR || type.type == mrs_msgs::EstimatorType::BRICKFLOW || type.type == mrs_msgs::EstimatorType::PAVEL) {
      return true;
    }
  ...
  bool Odometry::isValidType(const mrs_msgs::HeadingType &type) {
    ...
    if (type.type == mrs_msgs::HeadingType::PIXHAWK || type.type == mrs_msgs::HeadingType::GYRO || type.type == mrs_msgs::HeadingType::COMPASS ||
        type.type == mrs_msgs::HeadingType::OPTFLOW || type.type == mrs_msgs::HeadingType::HECTOR || type.type == mrs_msgs::HeadingType::BRICK || type.type == mrs_msgs::HeadingType::PAVEL ||
  ...
  std::string Odometry::printOdometryDiag() {
    ...
    if (type.type == mrs_msgs::EstimatorType::OPTFLOW) {
      ...
      s_diag += "HECTOR";
    } else if (type.type == mrs_msgs::EstimatorType::PAVEL) {
      s_diag += "PAVEL";
    ...
    if (hdg_type.type == mrs_msgs::HeadingType::PIXHAWK) {
      ...
      s_diag += "HECTOR";
    } else if (hdg_type.type == mrs_msgs::HeadingType::PAVEL) {
      s_diag += "PAVEL";

...
bool HeadingEstimator::getState(int state_id, Eigen::VectorXd &state) {
...

  {
    ...
    /* std::cout << "[HeadingEstimator]: " << m_estimator_name << " getting value: " << mp_lkf_x->getState(state_id) << " of state: " << state_id << std::endl;
     */
...
bool HeadingEstimator::setCovariance(const Eigen::MatrixXd &cov) {
  ...
  if (cov.rows() != m_n_states || cov.cols() != m_n_states) {
    std::cerr << "[HeadingEstimator]: " << m_estimator_name << ".setCovariance(const Eigen::MatrixXd &cov=" << cov

...
# 'name' 'command'
input=(
  'Rosbag' 'waitForRos; rosrun mrs_general record_ouster.sh
'
  'Sensors' 'waitForRos; roslaunch mrs_general sensors.launch
'
  'Control' 'waitForRos; roslaunch mrs_general core.launch
'
  'Ouster' 'waitForOdometry; roslaunch ouster_ros uav.launch
'
  'Aloam' 'waitForOdometry; sleep 15; roslaunch aloam_velodyne ouster_64_odometry.launch
'
  'MotorsOn' 'rosservice call /'"$UAV_NAME"'/control_manager/motors 1'
  'Takeoff' 'rosservice call /'"$UAV_NAME"'/uav_manager/takeoff'
  'ChangeEstimator' 'waitForOdometry; rosservice call /'"$UAV_NAME"'/odometry/change_odometry_source LIDAR'

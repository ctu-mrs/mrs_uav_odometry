#ifndef ALTITUDE_ESTIMATOR_ALOAMGARM2_H
#define ALTITUDE_ESTIMATOR_ALOAMGARM2_H
...
class AltitudeEstimatorAloamGarm2 : public AltitudeEstimator {

public:
  AltitudeEstimatorAloamGarm2(const std::string &estimator_name, const std::vector<bool> &fusing_measurement, const std::vector<var_alt_H_t> &H_multi, const var_alt_Q_t &Q,
  ...
  bool        getStates(algarm2_alt_x_t &x);
...
private:
  ...
  // repredictor
  std::unique_ptr<algarm2_rep_t> mp_rep;
  ...
  std::vector<algarm2_alt_H_t> m_H_multi;
  ...
  std::vector<algarm2_alt_R_t> m_R_multi;
  ...
  std::vector<std::shared_ptr<algarm2_alt_t>> mp_lkf_vector;

  // Variable for holding the current state and covariance
  algarm2_alt_statecov_t m_sc;
  ...
  algarm2_alt_H_t _H_range_biased_;
  algarm2_alt_H_t _H_slam_biased_;
  algarm2_alt_H_t _H_baro_biased_;
  float _q_biases_;
